...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('moderate_test.log'),
        logging.StreamHandler(sys.stdout)
    ]
)
logger = logging.getLogger('risk_test')

print('=== BẮT ĐẦU BACKTEST CHO MỨC RỦI RO moderate (5%) ===')
...
class BacktestEngine:
    def __init__(self, data, symbol, initial_balance=10000, leverage=5, risk_percentage=0.07):
        self.data = data.copy()
        self.symbol = symbol
        ...
        # Cấu hình rủi ro - mức moderate (5%)
...
# Tiến hành backtest
print("\n2. TIẾN HÀNH BACKTEST CHO MỨC RỦI RO moderate (5%)")

try:
    # Khởi tạo engine với mức rủi ro moderate (5%)
    engine = BacktestEngine(data, 'BTCUSDT', initial_balance=10000, leverage=5, risk_percentage=0.07)
    ...
    # Lưu kết quả
    result_path = result_dir / 'result_moderate.json'
    ...
    # Vẽ và lưu biểu đồ
    engine.plot_equity_curve(chart_dir / 'equity_moderate.png')
    engine.plot_trade_distribution(chart_dir / 'distribution_moderate.png')
