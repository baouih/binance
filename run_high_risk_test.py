...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('high_risk_test.log'),
        logging.StreamHandler(sys.stdout)
    ]
)
logger = logging.getLogger('risk_test')

print('=== BẮT ĐẦU BACKTEST CHO MỨC RỦI RO high_risk (5%) ===')
...
class BacktestEngine:
    def __init__(self, data, symbol, initial_balance=10000, leverage=5, risk_percentage=0.15):
        self.data = data.copy()
        self.symbol = symbol
        ...
        # Cấu hình rủi ro - mức high_risk (5%)
...
# Tiến hành backtest
print("\n2. TIẾN HÀNH BACKTEST CHO MỨC RỦI RO high_risk (5%)")

try:
    # Khởi tạo engine với mức rủi ro high_risk (5%)
    engine = BacktestEngine(data, 'BTCUSDT', initial_balance=10000, leverage=5, risk_percentage=0.15)
    ...
    # Lưu kết quả
    result_path = result_dir / 'result_high_risk.json'
    ...
    # Vẽ và lưu biểu đồ
    engine.plot_equity_curve(chart_dir / 'equity_high_risk.png')
    engine.plot_trade_distribution(chart_dir / 'distribution_high_risk.png')
