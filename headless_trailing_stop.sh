#!/bin/bash
# Script khởi động headless dịch vụ Trailing Stop (không yêu cầu tương tác người dùng)
...
if pgrep -f "python $PYTHON_SCRIPT" > /dev/null; then
    ...
    log "⚠️ Dịch vụ Trailing Stop đã đang chạy với PID $existing_pid. Tự động dừng..."
    kill $existing_pid
    sleep 2
...
# Khởi động dịch vụ
log "🚀 Đang khởi động dịch vụ Trailing Stop (chế độ headless)..."
