...
logger = logging.getLogger('comprehensive_risk_test')

class ComprehensiveRiskTester:
    """
    Kiểm tra toàn diện tất cả các cặp tiền và tất cả các mức độ rủi ro
    """
    def __init__(self):
        """Khởi tạo trình kiểm tra rủi ro toàn diện"""
        ...
        self.risk_levels = ['very_low', 'low', 'medium', 'high', 'very_high']
        ...
        # Thời gian backtest
        self.backtest_days = 90  # 3 tháng
        
    def run_all_risk_levels(self):
        """Chạy backtest cho tất cả các mức rủi ro"""
        results_by_risk = {}
        
        # Chạy backtest cho từng mức rủi ro
        for risk_level in self.risk_levels:
            logger.info(f"===== BẮT ĐẦU KIỂM TRA MỨC RỦI RO: {risk_level.upper()} =====")
    ...
    def generate_comparison_report(self, results_by_risk):
        ...
        # Lưu báo cáo so sánh
        report_file = f"{self.results_dir}risk_level_comparison_{timestamp}.txt"
        
        with open(report_file, 'w') as f:
            f.write("===== SO SÁNH HIỆU SUẤT CÁC MỨC ĐỘ RỦI RO =====\n\n")
            f.write("Thời gian backtest: 3 tháng gần nhất\n")
        ...
        logger.info(f"Đã tạo báo cáo so sánh tại {report_file}")
        
        # Tạo biểu đồ so sánh
        self.create_comparison_charts(df_comparison, timestamp)
        
        # In kết quả
        print("\n===== KẾT QUẢ KIỂM TRA TOÀN DIỆN CÁC MỨC RỦI RO =====")
    ...
    def create_comparison_charts(self, df_comparison, timestamp):
        ...
        try:
            ...
            # Lưu biểu đồ
            chart_file = f"{self.results_dir}risk_level_comparison_chart_{timestamp}.png"
...
if __name__ == "__main__":
    tester = ComprehensiveRiskTester()
