...
# Version 2.0 - Cải tiến độ ổn định và khả năng tự phục hồi
...
# Khởi động tiến trình watchdog trong nền
log_message "Khởi động watchdog để giám sát và tự khởi động lại SLTP Telegram khi cần..."
./sltp_watchdog.sh >> "sltp_watchdog.log" 2>&1 &
WATCHDOG_PID=$!

# Lưu PID của watchdog
echo $WATCHDOG_PID > "sltp_watchdog.pid"
log_message "Watchdog đã được khởi động với PID $WATCHDOG_PID"

# Chờ watchdog khởi động tiến trình chính
sleep 10

# Kiểm tra xem tiến trình chính đã chạy chưa
if [ -f "$PID_FILE" ]; then
    PID=$(cat $PID_FILE)
    if ps -p $PID > /dev/null; then
        log_message "SL/TP with Telegram Integration đang chạy bình thường với PID $PID"
        echo "SL/TP with Telegram Integration đang chạy bình thường với PID $PID"
        echo "Watchdog đang giám sát với PID $WATCHDOG_PID"
        echo "Sử dụng 'tail -f $LOG_FILE' để theo dõi log."
        
        # Thiết lập cronjob để kiểm tra watchdog định kỳ (mỗi 5 phút)
        (crontab -l 2>/dev/null; echo "*/5 * * * * if ! ps -p $(cat sltp_watchdog.pid 2>/dev/null) > /dev/null; then cd $(pwd) && ./auto_start_sltp_telegram.sh; fi") | crontab -
        log_message "Đã thiết lập cronjob để kiểm tra watchdog định kỳ"
    else
        log_message "SL/TP with Telegram Integration khởi động thất bại dù watchdog đã chạy."
        echo "SL/TP with Telegram Integration khởi động thất bại dù watchdog đã chạy."
        echo "Kiểm tra log file để biết thêm chi tiết: sltp_telegram_restart.log"
    fi
else
    log_message "Khởi động thất bại - không tìm thấy PID file sau 10 giây. Kiểm tra lỗi trong log."
    echo "Khởi động thất bại - không tìm thấy PID file sau 10 giây. Kiểm tra lỗi trong log."
    echo "Xem log file: sltp_telegram_restart.log và sltp_watchdog.log"
fi

# Hiển thị thêm thông tin hữu ích
echo "-------------------------------------"
echo "Hướng dẫn theo dõi và quản lý:"
echo "- Xem log chi tiết: tail -f $LOG_FILE"
echo "- Xem log watchdog: tail -f sltp_watchdog.log"
echo "- Dừng toàn bộ: kill -TERM $(cat sltp_watchdog.pid) && kill -TERM $(cat $PID_FILE)"
echo "-------------------------------------"