...
parser = argparse.ArgumentParser(description='Test một mức rủi ro cụ thể')
parser.add_argument('--risk_level', type=str, required=True, 
                    choices=['ultra_conservative', 'conservative', 'moderate', 'aggressive', 'high_risk', 'extreme_risk'],
                    help='Mức rủi ro cần test')
args = parser.parse_args()

# Thiết lập logging
log_file = f"{args.risk_level}_test.log"
logging.basicConfig(
    level=logging.INFO,
...
print(f'=== BẮT ĐẦU BACKTEST RIÊNG CHO MỨC RỦI RO {args.risk_level.upper()} ===')
...
# Copy code AdaptiveStrategy và BacktestEngine từ file gốc
...
# Tiến hành backtest
print(f"\n2. TIẾN HÀNH BACKTEST CHO MỨC RỦI RO {args.risk_level.upper()}")

try:
    # Khởi tạo engine với mức rủi ro được chọn
    engine = BacktestEngine(data, 'BTCUSDT', initial_balance=10000, leverage=5, risk_level=args.risk_level)
    ...
    # Lưu kết quả
    result_path = result_dir / f'result_{args.risk_level}.json'
    ...
    # Vẽ và lưu biểu đồ
    engine.plot_equity_curve(chart_dir / f'equity_{args.risk_level}.png')
    engine.plot_trade_distribution(chart_dir / f'distribution_{args.risk_level}.png')
