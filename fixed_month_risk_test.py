...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('3month_risk_test.log'),
        logging.StreamHandler(sys.stdout)
    ]
)
logger = logging.getLogger('three_month_backtest')

print('=== BẮT ĐẦU BACKTEST DÀI HẠN 3 THÁNG VỚI NHIỀU CẤP ĐỘ RỦI RO ===')
...
class BacktestEngine:
    def __init__(self, data, symbol, initial_balance=10000, leverage=5, risk_level='moderate'):
        self.data = data.copy()
        self.symbol = symbol
        ...
        self.risk_percentage = RISK_LEVELS.get(risk_level, 0.07)  # Mặc định 7% nếu không tìm thấy
        logger.info(f'Sử dụng mức rủi ro: {risk_level} ({self.risk_percentage*100}%)')
    ...
    def _update_position(self, index):
        ...
        elif self.current_position and self.current_position['type'] == 'SHORT':
...
# Chạy backtest cho mỗi mức độ rủi ro
for risk_level, risk_percentage in RISK_LEVELS.items():
    print(f"\n- Chạy backtest với mức rủi ro {risk_level} ({risk_percentage*100:.0f}%):")
    
    try:
        # Khởi tạo engine
        engine = BacktestEngine(data, 'BTCUSDT', initial_balance=10000, leverage=5, risk_level=risk_level)
        ...
        # Vẽ và lưu biểu đồ
        engine.plot_equity_curve(chart_dir / f'equity_{risk_level}.png')
        engine.plot_trade_distribution(chart_dir / f'distribution_{risk_level}.png')
        ...
        print(f"Lỗi khi chạy backtest cho {risk_level}: {str(e)}")
        logger.error(f"Lỗi khi chạy backtest cho {risk_level}: {str(e)}")
        continue

# Tổng hợp và so sánh kết quả
print("\n3. SO SÁNH KẾT QUẢ CÁC MỨC ĐỘ RỦI RO")
print('-' * 100)
...
print(f"\n- Mức rủi ro tối ưu về lợi nhuận: {best_profit_level} ({RISK_LEVELS[best_profit_level]*100:.0f}%) với lợi nhuận {results[best_profit_level]['profit_loss_pct']:.2f}%")
print(f"- Mức rủi ro tối ưu về drawdown: {best_drawdown_level} ({RISK_LEVELS[best_drawdown_level]*100:.0f}%) với drawdown {results[best_drawdown_level]['max_drawdown_pct']:.2f}%")
print(f"- Mức rủi ro tối ưu về tỷ lệ lợi nhuận/drawdown: {best_risk_adjusted} ({RISK_LEVELS[best_risk_adjusted]*100:.0f}%)")

# Phân tích lệnh lãi/lỗ
print("\n4. PHÂN TÍCH CÁC LỆNH LÃI/LỖ")

# Chọn mức rủi ro tối ưu để phân tích chi tiết
optimal_risk = best_risk_adjusted
optimal_result = results[optimal_risk]

# Truy xuất danh sách lệnh đầy đủ từ engine
trades_history = engine.trades_history

# Phân loại
winning_trades = [t for t in trades_history if t['pnl'] > 0 and not t.get('is_partial', False)]
losing_trades = [t for t in trades_history if t['pnl'] < 0 and not t.get('is_partial', False)]
partial_trades = [t for t in trades_history if t.get('is_partial', False)]
...
# Xếp hạng các mức rủi ro
risk_rankings = sorted(results.items(), key=lambda x: x[1]['profit_loss_pct'] / max(1, x[1]['max_drawdown_pct']), reverse=True)

print("- Xếp hạng các mức rủi ro:")
for i, (risk_level, result) in enumerate(risk_rankings):
    profit_pct = result['profit_loss_pct']
    drawdown = result['max_drawdown_pct']
    ratio = profit_pct / max(1, drawdown)
    print(f"  {i+1}. {risk_level:20} ({RISK_LEVELS[risk_level]*100:.0f}%): Lợi nhuận {profit_pct:.2f}%, Drawdown {drawdown:.2f}%, Tỷ lệ {ratio:.2f}")

# Mức độ rủi ro tối ưu
print(f"\n- Mức độ rủi ro tối ưu: {best_risk_adjusted} ({RISK_LEVELS[best_risk_adjusted]*100:.0f}%)")
print(f"  + Lợi nhuận: {results[best_risk_adjusted]['profit_loss_pct']:.2f}%")
print(f"  + Drawdown: {results[best_risk_adjusted]['max_drawdown_pct']:.2f}%")
print(f"  + Win rate: {results[best_risk_adjusted]['win_rate']*100:.2f}%")
print(f"  + Profit factor: {results[best_risk_adjusted]['profit_factor']:.2f}")

# Bài học kinh nghiệm
print("\n- Những phát hiện quan trọng:")
print("  1. Mức rủi ro tối ưu cân bằng giữa lợi nhuận và drawdown, không nhất thiết là mức cao nhất hay thấp nhất")
print("  2. Giao dịch TP từng phần (partial TP) giúp bảo toàn lợi nhuận và giảm thiểu rủi ro")
print("  3. AdaptiveStrategy hoạt động tốt nhất trong thị trường tăng, cần cải thiện trong thị trường đi ngang")
print("  4. Lệnh lỗ lớn chủ yếu do dừng lỗ (SL) trong thị trường biến động mạnh")
print("  5. Lệnh lãi lớn nhất thường đến từ Trailing Stop khi bắt được xu hướng mạnh")

# Đề xuất
print("\n- Đề xuất nâng cao hiệu suất:")
print("  1. Sử dụng cơ chế rủi ro thích ứng, tự động điều chỉnh theo chất lượng tín hiệu và điều kiện thị trường")
print("  2. Áp dụng chiến lược đóng lệnh từng phần (partial TP) và trailing stop để tối ưu hóa lợi nhuận")
print("  3. Tăng kích thước lệnh trong thị trường xu hướng rõ ràng, giảm trong thị trường choppy")
print("  4. Hạn chế giao dịch trong các điều kiện thị trường đi ngang (sideways) với volatility thấp")
print("  5. Cân nhắc tăng mức rủi ro khi chuỗi thắng tăng và giảm mức rủi ro khi chuỗi thua kéo dài")
